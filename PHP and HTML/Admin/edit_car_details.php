...
// ── Determine car_id (edit vs new) ──
// This is the SINGLE source of truth for which car we're editing.
...
if ($car_id > 0) {
    ...
    $result = mysqli_query($conn, "SELECT c.*, stat.car_status_price, stat.car_status_status, stat.car_status_stock_quantity, ct.car_type_name, loc.location_state, loc.location_city,
...
// ── SAVE handler ──
// KEY FIX: Use $car_id (from GET) directly — not $_POST['car_id_post'].
// $car_id > 0 = edit/update, $car_id == 0 = new insert.
if (isset($_POST['save_all_details'])) {
    try {
        mysqli_begin_transaction($conn);
        $is_updating = ($car_id > 0);

        $feat_safety = isset($_POST['feat_safety']) && is_array($_POST['feat_safety']) ? implode(',', $_POST['feat_safety']) : '';
        $feat_tech = isset($_POST['feat_tech']) && is_array($_POST['feat_tech']) ? implode(',', $_POST['feat_tech']) : '';
        $feat_comf = isset($_POST['feat_comf']) && is_array($_POST['feat_comf']) ? implode(',', $_POST['feat_comf']) : '';

        $brand = mysqli_real_escape_string($conn, trim($_POST['brand'] ?? 'Proton'));
        $model = mysqli_real_escape_string($conn, trim($_POST['model'] ?? ''));
        $variant = mysqli_real_escape_string($conn, trim($_POST['variant'] ?? ''));
        ...
        $nf = function ($v) {
            $x = max(0, (float) $v);
            return $x > 0 ? $x : 'NULL'; };
        $displacement = $ni($_POST['displacement'] ?? 0);
        $hp = $ni($_POST['hp'] ?? 0);
        $torque = $ni($_POST['torque'] ?? 0);
        $acceleration = $nf($_POST['acceleration'] ?? 0);
        $fuel_consump = $nf($_POST['fuel_consumption'] ?? 0);
        $battery = $ni($_POST['battery_range'] ?? 0);
        $cylinders = $ni($_POST['cylinders'] ?? 0);
        $top_speed = $ni($_POST['top_speed'] ?? 0);
        $co2_emissions = $ni($_POST['co2_emissions'] ?? 0);
        $wheelbase = $ni($_POST['wheelbase'] ?? 0);
        $boot_cap = $ni($_POST['boot_cap'] ?? 0);
        $fuel_tank = $ni($_POST['fuel_tank'] ?? 0);
        $weight = $ni($_POST['weight'] ?? 0);
        $seats = $ni($_POST['seats'] ?? 0);
        $width_with_mirrors = $ni($_POST['width_with_mirrors'] ?? 0);
        $ground_clearance = $ni($_POST['ground_clearance'] ?? 0);
        $airbags_count = $ni($_POST['airbags_count'] ?? 0);
        ...
        $plate = mysqli_real_escape_string($conn, trim($_POST['plate_no'] ?? ''));
        ...
        $type_id = (mysqli_num_rows($type_query) > 0) ? (int) mysqli_fetch_assoc($type_query)['car_type_id'] : 1;
        ...
        if ($is_updating) {
            mysqli_query($conn, "UPDATE cars SET 
                ...
                WHERE car_id=$car_id") or throw new Exception(mysqli_error($conn));
            ...
            $target_car_id = (int) mysqli_insert_id($conn);
        }
        ...
        if ($origin === 'New Car') {
            ...
            if (isset($_POST['inv_color']) && is_array($_POST['inv_color'])) {
                for ($i = 0; $i < count($_POST['inv_color']); $i++) {
                    $c_name = trim($_POST['inv_color'][$i] ?? '');
                    if (empty($c_name))
                        continue;
                    $c_name = mysqli_real_escape_string($conn, $c_name);
                    $c_hex = mysqli_real_escape_string($conn, $_POST['inv_color_hex'][$i] ?? '#ffffff');
                    $q = max(0, (int) ($_POST['inv_qty'][$i] ?? 0));
                    mysqli_query($conn, "INSERT INTO car_inventory (car_id, variant_name, color_name, color_hex, quantity) VALUES ($target_car_id, '$variant', '$c_name', '$c_hex', $q)");
                    $total_stock += $q;
                }
            }
        ...
        if (!$is_updating && isset($_SESSION['temp_car_images']) && !empty($_SESSION['temp_car_images'])) {
            ...
            foreach ($_SESSION['temp_car_images'] as $temp_path) {
                if (file_exists($temp_path)) {
                    $new_path = $real_upload_dir . basename($temp_path);
                    if (rename($temp_path, $new_path)) {
                        $np = mysqli_real_escape_string($conn, $new_path);
                        mysqli_query($conn, "INSERT INTO car_image (car_id, car_image_url) VALUES ($target_car_id, '$np')");
                    }
                }
            }
            unset($_SESSION['temp_car_images']);
        ...
        // Show error at top instead of dying, so user can see what went wrong
        $save_error = $e->getMessage();
    }
}

$saved_safety = array_filter(explode(',', $car['feat_safety'] ?? ''));
$saved_tech = array_filter(explode(',', $car['feat_tech'] ?? ''));
$saved_comf = array_filter(explode(',', $car['feat_comf'] ?? ''));
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <main class="main-content">
        <?php if (!empty($save_error)): ?>
            <div
                style="background:#fee2e2;color:#991b1b;padding:16px;border-radius:8px;margin:16px;border:1px solid #f87171;">
                <strong>❌ Save failed:</strong> <?= htmlspecialchars($save_error) ?>
            </div>
        <?php endif; ?>

        <!-- NOTE: novalidate disables browser HTML5 validation which silently blocks submit when fields in hidden tabs are "required". We do our own validation in JS. -->
        <form id="mainCarForm" method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="inner-tabs">
                ...
                <button type="button" id="tab-history-btn" class="inner-tab-btn" data-target="tab-history"
                    ...
                    <i class="fas fa-history"></i> Used Car History
                </button>
            </div>
                    ...
                    <div class="grid-4">
                        ...
                        <div class="form-group"><label>Body Type</label>
                            <select name="body_type" class="form-control">
                                <?php foreach (['Sedan', 'SUV', 'Hatchback', 'EV', 'MPV'] as $bt): ?>
                                    <option <?= ($car['car_type_name'] ?? '') == $bt ? 'selected' : '' ?>><?= $bt ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group"><label>Status</label>
                            <select name="status" class="form-control">
                                <?php foreach (['Active', 'Inactive', 'Draft'] as $st): ?>
                                    <option <?= ($car['car_status_status'] ?? '') == $st ? 'selected' : '' ?>><?= $st ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    ...
                    <div class="grid-3">
                        ...
                        <div class="form-group"><label>Negotiable</label>
                            <select name="negotiable" class="form-control">
                                ...
                                <option value="No" <?= (($car['negotiable'] ?? '') != 'Yes') ? 'selected' : '' ?>>No
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="form-group"><label>Promotion / Rebate Details</label><input type="text"name="promotion_rebate" class="form-control" value="<?= htmlspecialchars($car['promotion_rebate'] ?? '') ?>"></div>
                    <div class="form-group"><label>Promo Valid Until</label><input type="date" name="promo_valid_until" class="form-control" value="<?= htmlspecialchars($car['promo_valid_until'] ?? '') ?>"></div>
                    <div class="form-group" id="stock_group"><label>Stock Available (units)</label><input type="number" min="0" max="1" name="stock" class="form-control" value="<?= htmlspecialchars($car['car_status_stock_quantity'] ?? 1) ?>"></div>
                </div>
            </div>
            ...
            <div class="form-section" id="variant_inventory_section">
                ...
                <div id="inventory-container">
                    ...
                    if ($is_edit && ($car['car_origin'] ?? '') == 'New Car') {
                        $inv_q = mysqli_query($conn, "SELECT * FROM car_inventory WHERE car_id = $car_id ORDER BY color_name ASC");
                        while ($inv_row = mysqli_fetch_assoc($inv_q)) {
                            $c = addslashes($inv_row['color_name']);
                            $hex = addslashes($inv_row['color_hex'] ?: '#ffffff');
                            $q = (int) $inv_row['quantity'];
                            echo "<script>document.addEventListener('DOMContentLoaded', function() { addInventoryRow('$c', '$hex', $q); });</script>";
                        }
                    }
                    ?>
                </div>
                <button type="button" class="btn-add-blue" style="margin-top:10px;border:none;" onclick="addInventoryRow()">
        ...
        <div id="tab-specs" class="tab-content">
                ...
                <div class="grid-4">
                    <div class="form-group"><label>Engine Type</label>
                        <select name="engine_type" class="form-control">
                            <?php foreach (['Petrol', 'Hybrid', 'EV'] as $et): ?>
                                <option <?= ($car['engine_type'] ?? '') == $et ? 'selected' : '' ?>><?= $et ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group"><label>Displacement (cc)</label><input type="number" min="0" name="displacement" class="form-control" value="<?= htmlspecialchars($car['displacement'] ?? '') ?>"></div>
                    <div class="form-group"><label>Horsepower (hp)</label><input type="number" min="0" name="hp" class="form-control" value="<?= htmlspecialchars($car['hp'] ?? '') ?>"></div>
                    <div class="form-group"><label>Torque (Nm)</label><input type="number" min="0" name="torque" class="form-control" value="<?= htmlspecialchars($car['torque'] ?? '') ?>"></div>
                    <div class="form-group"><label>0-100 km/h (s)</label><input type="number" step="0.1" min="0" name="acceleration" class="form-control" value="<?= htmlspecialchars($car['acceleration'] ?? '') ?>"></div>
                    <div class="form-group"><label>Transmission</label>
                        <select name="transmission" class="form-control">
                            <?php foreach (['Auto', 'Manual', 'CVT', 'DCT'] as $tr): ?>
                    ...
                    <div class="form-group"><label>Drive Type</label>
                        <select name="drive_type" class="form-control">
                            <?php foreach (['FWD', 'RWD', 'AWD'] as $dt): ?>
                                <option <?= ($car['drive_type'] ?? '') == $dt ? 'selected' : '' ?>><?= $dt ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group"><label>Fuel Type</label>
                        <select name="fuel_type" class="form-control">
                            <?php foreach (['RON95', 'RON97', 'Diesel', 'Electric'] as $ft): ?>
                                <option <?= ($car['fuel_type'] ?? '') == $ft ? 'selected' : '' ?>><?= $ft ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group"><label>Fuel Consumption</label><input type="number" step="0.1" min="0" name="fuel_consumption" class="form-control" value="<?= htmlspecialchars($car['fuel_consumption'] ?? '') ?>"></div>
                    <div class="form-group"><label>Battery Range (km)</label><input type="number" min="0" name="battery_range" class="form-control" value="<?= htmlspecialchars($car['battery_range'] ?? '') ?>"></div>
                    <div class="form-group"><label>No. of Cylinders</label><input type="number" min="0" name="cylinders" class="form-control" value="<?= htmlspecialchars($car['cylinders'] ?? '') ?>"></div>
                    <div class="form-group"><label>Top Speed (km/h)</label><input type="number" min="0" name="top_speed" class="form-control" value="<?= htmlspecialchars($car['top_speed'] ?? '') ?>"></div>
                    <div class="form-group"><label>CO₂ Emissions</label><input type="number" min="0" name="co2_emissions" class="form-control" value="<?= htmlspecialchars($car['co2_emissions'] ?? '') ?>"></div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="grid-3">
                    <div class="form-group"><label>L × W × H (mm)</label><input type="text" name="dimensions" class="form-control" value="<?= htmlspecialchars($car['dimensions'] ?? '') ?>"></div>
                    <div class="form-group"><label>Wheelbase (mm)</label><input type="number" min="0" name="wheelbase" class="form-control" value="<?= htmlspecialchars($car['wheelbase'] ?? '') ?>"></div>
                    <div class="form-group"><label>Boot Capacity (L)</label><input type="number" min="0" name="boot_cap" class="form-control" value="<?= htmlspecialchars($car['boot_cap'] ?? '') ?>"></div>
                    <div class="form-group"><label>Fuel Tank (L)</label><input type="number" min="0" name="fuel_tank" class="form-control" value="<?= htmlspecialchars($car['fuel_tank'] ?? '') ?>"></div>
                    <div class="form-group"><label>Kerb Weight (kg)</label><input type="number" min="0" name="weight" class="form-control" value="<?= htmlspecialchars($car['weight'] ?? '') ?>"></div>
                    <div class="form-group"><label>Seating Capacity</label><input type="number" min="0" name="seats" class="form-control" value="<?= htmlspecialchars($car['seats'] ?? '') ?>"></div>
                    <div class="form-group"><label>Width with Mirrors (mm)</label><input type="number" min="0" name="width_with_mirrors" class="form-control" value="<?= htmlspecialchars($car['width_with_mirrors'] ?? '') ?>"></div>
                    <div class="form-group"><label>Ground Clearance (mm)</label><input type="number" min="0" name="ground_clearance" class="form-control" value="<?= htmlspecialchars($car['ground_clearance'] ?? '') ?>"></div>
                    <div class="form-group"><label>Airbags Count</label><input type="number" min="0" name="airbags_count" class="form-control" value="<?= htmlspecialchars($car['airbags_count'] ?? '') ?>"></div>
                </div>
            </div>
                    ...
                    <div class="form-group" id="ext_color_group">
                        ...
                        <div style="display:flex;align-items:center;gap:10px;">
                            <div class="custom-color-picker" style="flex-shrink:0;">
                                ...
                                <div class="selected-color-circle" id="ext_color_circle" style="background:#ffffff;width:42px;height:42px;" onclick="toggleColorPalette(this)" title="Click to pick a colour"></div>
                                <div class="color-palette-popup">
                                    <div class="palette-swatch" style="background:#ffffff;" onclick="selectExtColor(this,'#ffffff','Solid White')" title="Solid White"></div>
                                    <div class="palette-swatch" style="background:radial-gradient(circle,#ffffff 0%,#f3f4f6 100%);" onclick="selectExtColor(this,'#f8fafc','Pearl White')" title="Pearl White"></div>
                                    <div class="palette-swatch" style="background:linear-gradient(135deg,#f3f4f6,#9ca3af);" onclick="selectExtColor(this,'#d1d5db','Silver')" title="Silver"></div>
                                    <div class="palette-swatch" style="background:#6b7280;" onclick="selectExtColor(this,'#6b7280','Meteor Grey')" title="Meteor Grey"></div>
                                    <div class="palette-swatch" style="background:#111827;" onclick="selectExtColor(this,'#111827','Black')" title="Black"></div>
                                    <div class="palette-swatch" style="background:radial-gradient(circle,#374151 0%,#111827 100%);" onclick="selectExtColor(this,'#1f2937','Matte Black')" title="Matte Black"></div>
                                    <div class="palette-swatch" style="background:#ef4444;" onclick="selectExtColor(this,'#ef4444','Ruby Red')" title="Ruby Red"></div>
                                    <div class="palette-swatch" style="background:#991b1b;" onclick="selectExtColor(this,'#991b1b','Maroon')" title="Maroon"></div>
                                    <div class="palette-swatch" style="background:#f97316;" onclick="selectExtColor(this,'#f97316','Orange')" title="Orange"></div>
                                    <div class="palette-swatch" style="background:#eab308;" onclick="selectExtColor(this,'#eab308','Yellow')" title="Yellow"></div>
                                    <div class="palette-swatch" style="background:linear-gradient(135deg,#fde047,#d97706);" onclick="selectExtColor(this,'#d97706','Champagne Gold')" title="Champagne Gold"></div>
                                    <div class="palette-swatch" style="background:#b45309;" onclick="selectExtColor(this,'#b45309','Bronze')" title="Bronze"></div>
                                    <div class="palette-swatch" style="background:#3b82f6;" onclick="selectExtColor(this,'#3b82f6','Ocean Blue')" title="Ocean Blue"></div>
                                    <div class="palette-swatch" style="background:#0ea5e9;" onclick="selectExtColor(this,'#0ea5e9','Cyan')" title="Cyan"></div>
                                    <div class="palette-swatch" style="background:#1e3a8a;" onclick="selectExtColor(this,'#1e3a8a','Navy Blue')" title="Navy Blue"></div>
                                    <div class="palette-swatch" style="background:#22c55e;" onclick="selectExtColor(this,'#22c55e','Green')" title="Green"></div>
                                    <div class="palette-swatch" style="background:#064e3b;" onclick="selectExtColor(this,'#064e3b','Dark Green')" title="Dark Green"></div>
                                    <div class="palette-swatch" style="background:#8b5cf6;" onclick="selectExtColor(this,'#8b5cf6','Purple')" title="Purple"></div>
                                </div>
                            </div>
                            <input type="text" name="ext_color" id="ext_color_input" class="form-control" placeholder="e.g., Silver" value="<?= htmlspecialchars($car['ext_color'] ?? '') ?>" style="flex:1;margin-bottom:0;">
                        </div>
                    </div>
                    <div class="form-group"><label>Interior Colour</label><input type="text" name="int_color" class="form-control" value="<?= htmlspecialchars($car['int_color'] ?? '') ?>"></div>
                    <div class="form-group"><label>Seat Material</label>
                        <select name="seat_mat" class="form-control">
                            <?php foreach (['Fabric', 'Leather', 'Half-leather'] as $sm): ?>
                                <option <?= ($car['seat_mat'] ?? '') == $sm ? 'selected' : '' ?>><?= $sm ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group"><label>Wheel Size &amp; Type</label><input type="text" name="wheel_size" class="form-control" value="<?= htmlspecialchars($car['wheel_size'] ?? '') ?>"></div>
                    <div class="form-group"><label>Headlights</label>
                        <select name="headlights" class="form-control">
                            <?php foreach (['Halogen', 'LED', 'Matrix LED'] as $hl): ?>
                                <option <?= ($car['headlights'] ?? '') == $hl ? 'selected' : '' ?>><?= $hl ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group"><label>Infotainment Screen (inch)</label><input type="text" name="screen" class="form-control" value="<?= htmlspecialchars($car['screen'] ?? '') ?>"></div>
                </div>
            </div>
        </div>
        ...
        <div id="tab-media" class="tab-content">
            <div class="form-section">
                <h3 class="section-header"><i class="fas fa-camera"></i> General Car Gallery <span style="color:#9ca3af;font-weight:400;margin-left:10px;font-size:14px;">(Changes are instant)</span></h3>
                <input type="file" id="ajaxImageInput" name="ajax_car_images[]" multiple accept="image/*" style="display:none;">
                    ...
                    <?php if ($is_edit && !empty($car_images)): ?>
                            <?php foreach ($car_images as $img): ?>
                                <div class="gallery-item" id="img_container_<?= $img['car_image_id'] ?>">
                                    ...
                                    <button type="button" class="delete-img-btn" onclick="deleteExistingImage(<?= $img['car_image_id'] ?>)" title="Delete image"><i class="fas fa-trash-alt"></i></button>
                                </div>
                            <?php endforeach; ?>
                    <?php endif; ?>
                    <?php if (!$is_edit && isset($_SESSION['temp_car_images']) && !empty($_SESSION['temp_car_images'])): ?>
                            <?php foreach ($_SESSION['temp_car_images'] as $index => $temp_path): ?>
                                <div class="gallery-item" id="img_container_temp_<?= $index ?>">
                                    ...
                                    <button type="button" class="delete-img-btn" onclick="deleteTempImage('temp_<?= $index ?>')" title="Delete image"><i class="fas fa-trash-alt"></i></button>
                                </div>
                            <?php endforeach; ?>
                    <?php endif; ?>
                </div>
            ...
            <div class="form-section">
                ...
                <div class="checkbox-group">
                    <label class="checkbox-item"><input type="checkbox" name="feat_safety[]" value="ABS"            <?= in_array('ABS', $saved_safety) ? 'checked' : '' ?>> ABS</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_safety[]" value="ESC"            <?= in_array('ESC', $saved_safety) ? 'checked' : '' ?>> ESC</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_safety[]" value="AEB"            <?= in_array('AEB', $saved_safety) ? 'checked' : '' ?>> AEB</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_safety[]" value="LKA"            <?= in_array('LKA', $saved_safety) ? 'checked' : '' ?>> Lane Keep Assist</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_safety[]" value="BSM"            <?= in_array('BSM', $saved_safety) ? 'checked' : '' ?>> Blind Spot</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_safety[]" value="360 Cam"        <?= in_array('360 Cam', $saved_safety) ? 'checked' : '' ?>> 360° Camera</label>
                </div>
                <label style="font-weight:700;color:#111827;margin-top:24px;display:block;">Technology &amp; Comfort</label>
                <div class="checkbox-group">
                    <label class="checkbox-item"><input type="checkbox" name="feat_tech[]" value="Apple CarPlay"    <?= in_array('Apple CarPlay', $saved_tech) ? 'checked' : '' ?>> Apple CarPlay</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_tech[]" value="Android Auto"     <?= in_array('Android Auto', $saved_tech) ? 'checked' : '' ?>> Android Auto</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_tech[]" value="Keyless"          <?= in_array('Keyless', $saved_tech) ? 'checked' : '' ?>> Keyless Entry</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_tech[]" value="Wireless Charging"<?= in_array('Wireless Charging', $saved_tech) ? 'checked' : '' ?>> Wireless Charging</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_tech[]" value="HUD"              <?= in_array('HUD', $saved_tech) ? 'checked' : '' ?>> HUD</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_tech[]" value="Push Start"       <?= in_array('Push Start', $saved_tech) ? 'checked' : '' ?>> Push Start</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_comf[]" value="Sunroof"          <?= in_array('Sunroof', $saved_comf) ? 'checked' : '' ?>> Sunroof</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_comf[]" value="Leather Seats"    <?= in_array('Leather Seats', $saved_comf) ? 'checked' : '' ?>> Leather Seats</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_comf[]" value="Electric Tailgate"<?= in_array('Electric Tailgate', $saved_comf) ? 'checked' : '' ?>> Electric Tailgate</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_comf[]" value="Heated Seats"     <?= in_array('Heated Seats', $saved_comf) ? 'checked' : '' ?>> Heated Seats</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_comf[]" value="Memory Seat"      <?= in_array('Memory Seat', $saved_comf) ? 'checked' : '' ?>> Memory Seat</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_comf[]" value="Auto AC"          <?= in_array('Auto AC', $saved_comf) ? 'checked' : '' ?>> Auto AC</label>
                    <label class="checkbox-item"><input type="checkbox" name="feat_comf[]" value="Ambient Lighting" <?= in_array('Ambient Lighting', $saved_comf) ? 'checked' : '' ?>> Ambient Lighting</label>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control" rows="6"><?= htmlspecialchars($car['description'] ?? '') ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div id="tab-history" class="tab-content">
            <div class="form-section" style="border-color:#fdba74;background:#fffbeb;">
                <h3 class="section-header" style="color:#c2410c;border-bottom-color:#fed7aa;"><i class="fas fa-file-contract"></i> Vehicle History &amp; Location</h3>
                <div class="grid-3">
                    <div class="form-group"><label>Plate Number</label><input type="text" name="plate_no" class="form-control" value="<?= htmlspecialchars($car['car_plate'] ?? '') ?>"></div>
                    ...
                    <div class="form-group"><label>City</label>
                        <select id="city_select" name="location_city" class="form-control">
                            <option value="">Select City...</option>
                        </select>
                        <input type="hidden" id="saved_state" value="<?= htmlspecialchars($car['location_state'] ?? '') ?>">
                        <input type="hidden" id="saved_city"  value="<?= htmlspecialchars($car['location_city'] ?? '') ?>">
                    </div>
                    <div class="form-group"><label>Mileage (km)</label><input type="number" min="0" name="used_mileage" class="form-control" value="<?= htmlspecialchars($car['used_mileage'] ?? '') ?>"></div>
                    <div class="form-group"><label>Previous Owners</label><input type="number" min="0" name="owners" class="form-control" value="<?= htmlspecialchars($car['owners'] ?? '') ?>"></div>
                    <div class="form-group"><label>Accident History</label>
                        <select name="accident" class="form-control">
                            <?php foreach (['None', 'Minor', 'Major'] as $ac): ?>
                                <option <?= ($car['accident'] ?? 'None') == $ac ? 'selected' : '' ?>><?= $ac ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group"><label>Flood/Fire History</label>
                        <select name="flood_fire_history" class="form-control">
                            <option value="None"  <?= (($car['flood'] ?? '') == 'None') ? 'selected' : '' ?>>None</option>
                            <option value="Flood" <?= (($car['flood'] ?? '') == 'Flood') ? 'selected' : '' ?>>Flood Record</option>
                            <option value="Fire"  <?= (($car['flood'] ?? '') == 'Fire') ? 'selected' : '' ?>>Fire Record</option>
                        </select>
                    </div>
                    <div class="form-group"><label>Road Tax Expiry</label><input type="date" name="road_tax_expiry" class="form-control" value="<?= htmlspecialchars($car['roadtax'] ?? '') ?>"></div>
                    <div class="form-group"><label>Puspakom Date</label><input type="date" name="puspakom_date" class="form-control" value="<?= htmlspecialchars($car['puspakom'] ?? '') ?>"></div>
                    <div class="form-group" style="grid-column:span 3;"><label>Known Issues / Defects</label><input type="text" name="known_issues" class="form-control" value="<?= htmlspecialchars($car['defects'] ?? '') ?>"></div>
                </div>
            </div>

            <div class="form-section">
                <h3 class="section-header"><i class="fas fa-clipboard-list"></i> Admin Condition Scoring (Internal)</h3>
                <div class="grid-4">
                    <div class="form-group"><label>Exterior (0-5)</label><input type="number" id="score_ext" name="score_ext" class="form-control score-input" min="0" max="5" value="<?= htmlspecialchars($car['score_ext'] ?? 5) ?>"></div>
                    <div class="form-group"><label>Interior (0-5)</label><input type="number" id="score_int" name="score_int" class="form-control score-input" min="0" max="5" value="<?= htmlspecialchars($car['score_int'] ?? 5) ?>"></div>
                    <div class="form-group"><label>Mechanical (0-5)</label><input type="number" id="score_mech" name="score_mech" class="form-control score-input" min="0" max="5" value="<?= htmlspecialchars($car['score_mech'] ?? 5) ?>"></div>
                    <div class="form-group"><label>Tyre (0-5)</label><input type="number" id="score_tyre" name="score_tyre" class="form-control score-input" min="0" max="5" value="<?= htmlspecialchars($car['score_tyre'] ?? 5) ?>"></div>
                </div>
                <div class="grid-2" style="margin-top:24px;">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="inspector_notes" class="form-control" rows="3"><?= htmlspecialchars($car['inspector_notes'] ?? '') ?></textarea>
                    </div>
                </div>
            </div>
        </div>
...
<!-- Pass car_id to JS for AJAX image uploads -->

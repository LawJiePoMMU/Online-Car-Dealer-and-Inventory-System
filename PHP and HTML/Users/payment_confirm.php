...
if ($payment_id <= 0) {
    show_error_page(
        'Receipt Not Found',
        'No valid payment reference was provided. Your receipt link may have expired.',
        'Return Home',
        'index.php'
    );
}
...
SELECT
    ...
    b.snapshot_data
FROM payments p
INNER JOIN bookings b ON b.booking_id = p.reference_id
WHERE p.payment_id = ?
AND b.user_id = ?
LIMIT 1
";
...
if (!$data) {
    show_error_page(
        ...
        'This payment record does not belong to your account or could not be found.',
        'Return Home',
        'index.php'
    );
}
...
$snap = json_decode($data['snapshot_data'] ?: '{}', true);
...
$car_image   = !empty($snap['car_image'])
    ? $snap['car_image']
...
if (!empty($data['payment_date'])) {
    ...
    if ($ts) $payment_date = date('d M Y, h:i A', $ts);
}
...
// Approval message for finance types
$approval_message = '';
if ($payment_type === 'Booking Fee') {
    $approval_message =
        'Your booking fee has been received successfully. Our financing team will review your submitted documents and contact you within 1-2 business days regarding loan approval and the next steps.';
}
elseif ($payment_type === 'Down Payment') {
    $approval_message =
        'Your down payment has been received. The financing team will verify your insurance cover note before generating your monthly installment schedule.';
}
...
@media(max-width:700px){ .vehicle-section{ grid-template-columns:1fr; } }
.vehicle-section img{
    width:100%;
    height:180px;
    object-fit:cover;
...
.info-card.mono p{ font-family:'Courier New',monospace; font-size:13px; }
...
.note-box p strong{ color:#1d4ed8; }
...
@media print{
    body{ background:#fff; }
    .button-group, .topbar, nav, header{ display:none !important; }
    .success-container{ box-shadow:none; border:1px solid #ddd; }
}
</style>
    ...
    <div class="success-container">
        ...
        <div class="content-wrapper">
            ...
            <div class="detail-section">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-card" style="grid-column:1 / -1;">
                        <label>Remarks</label>
                        <p style="font-weight:normal;font-size:13px;color:#475569;"><?= htmlspecialchars($remarks) ?></p>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="detail-section">
                ...
                <div class="info-grid">
                    <div class="info-card">
                        ...
                        <p style="color:<?= $booking_status === 'Approved' ? '#16a34a' : ($booking_status === 'Rejected' ? '#dc2626' : '#d97706') ?>;">
            ...
            <div class="button-group">
                ...
                <a href="javascript:window.print()" class="action-btn secondary-btn" style="flex:0;min-width:auto;padding:14px 18px;">

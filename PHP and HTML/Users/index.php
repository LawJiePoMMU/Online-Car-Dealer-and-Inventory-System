...
$sql_models = "
    SELECT 
        c.car_model, 
        c.car_brand,
        (SELECT ci.car_image_url
         FROM cars c2
         JOIN car_image ci ON ci.car_id = c2.car_id
         WHERE c2.car_model = c.car_model AND c2.car_brand = c.car_brand
         ORDER BY c2.car_id ASC LIMIT 1) AS model_image
    FROM cars c
...
<style>
    ...
    .model-card-overlay {
        position: absolute;
        inset: 0;
        background: linear-gradient(to top, rgba(15, 23, 42, 0.95) 0%, rgba(15, 23, 42, 0.3) 45%, rgba(15, 23, 42, 0) 100%);
        display: flex;
        ...
        <div class="models-slider-container" id="modelsSlider">
            ...
            if (count($all_models) > 0) {
                foreach ($all_models as $m):
                    ...
                    $fallback_img = isset($premium_images[$key]) ? $premium_images[$key] : 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=800&q=80';
                    $img = !empty($m['model_image']) ? htmlspecialchars($m['model_image']) : $fallback_img;
                    ?>
                    <a href="cars.php?model=<?php echo urlencode($modelName); ?>" class="premium-model-card">
                        <img src="<?php echo $img; ?>" alt="<?php echo $displayName; ?>"
                            onerror="this.onerror=null; this.src='https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=800&q=80';">

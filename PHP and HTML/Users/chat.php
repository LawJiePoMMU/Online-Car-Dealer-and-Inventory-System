    ...
    if ($action == 'fetch_users') {
        $sql_users = "SELECT DISTINCT u.user_id, u.user_name, u.user_phone, u.user_avatar,
            ...
            LEFT JOIN chat_state cs
                   ON cs.user_id = $my_id AND cs.target_id = u.user_id AND cs.chat_type = 'user'
            LEFT JOIN muted_chats mc
                   ON mc.user_id = $my_id AND mc.muted_target_id = u.user_id AND mc.chat_type = 'user'
            WHERE u.user_id != $my_id
              AND (m.sender_id = $my_id OR m.receiver_id = $my_id)
              AND (
                    cs.deleted_at IS NULL
                    OR m.created_at > cs.deleted_at
              )";
...
WHERE gm.user_id=$my_id";
    ...
    if ($action == 'fetch_messages') {
        ...
        $chat_type = safe_chat_type(isset($_POST['chat_type']) ? $_POST['chat_type'] : 'user');
        if ($chat_type === 'user') {
            mysqli_query($conn, "UPDATE messages SET is_read = 1 WHERE sender_id = $other_id AND receiver_id = $my_id AND (is_read = 0 OR is_read IS NULL)");
        }
        $msgs = [];
if ($chat_type === 'group') {
    ...
    if ($action == 'create_group') {
        if ($my_role !== 'Admin') {
            echo json_encode(['status' => 'error', 'message' => 'You are not authorized to create groups.']);
            exit();
        }
    ...
    if ($action == 'toggle_mute') {
        $other_id = isset($_POST['other_id']) ? (int) $_POST['other_id'] : 0;
        $chat_type = safe_chat_type(isset($_POST['chat_type']) ? $_POST['chat_type'] : 'user');
        $is_muted = isset($_POST['is_muted']) ? $_POST['is_muted'] : 'false';

        if ($is_muted === 'true') {
            mysqli_query($conn, "INSERT IGNORE INTO muted_chats (user_id, muted_target_id, chat_type)
                                 VALUES ($my_id, $other_id, '$chat_type')");
        } else {
            mysqli_query($conn, "DELETE FROM muted_chats
                                 WHERE user_id = $my_id AND muted_target_id = $other_id AND chat_type = '$chat_type'");
        }
        echo json_encode(['status' => 'success']);
        exit();
    }
...
<body>
    ...
    <div class="main-content" style="padding: 24px; min-height: 100vh; background: #f3f4f6;">
        <header class="topbar" style="margin-bottom: 24px; max-width: 1200px; margin-left: auto; margin-right: auto;">
            <div class="page-title">
                <h1 style="font-size: 24px; font-weight: 700; color: #111827;">Chats</h1>
            </div>
        </header>

        <div class="chat-container" style="max-width: 1200px; margin: 0 auto;">
                ...
                <div class="chat-search-wrap">
                    <div class="search-bar-row">
                        ...
                        <button class="btn-add-blue" onclick="openNewChatModal()"
                            style="padding: 10px; border-radius: 8px; background: #1e3a8a; color: #fff; border: none; cursor: pointer;"
            ...
            <div class="chat-main" id="mainChatArea" style="display:none;">
                <div class="chat-header" id="chatHeader" onclick="toggleProfileSidebar()">
                    ...
                    <div class="action-icons" style="display:flex; gap: 30px; align-items: center;">
                        <?php if ($my_role == 'Admin'): ?>
            ...
            <div class="chat-main" id="emptyChatArea"
                style="align-items: center; justify-content: center; background: linear-gradient(135deg, #f8fafc 0%, #e0e7ff 100%);">
                <div style="text-align: center; color: #9ca3af;">
                    <i class="fas fa-comments" style="font-size: 60px; margin-bottom: 16px; opacity: 0.5;"></i>
                    <h3 style="color: #4b5563;">Select a contact
                        <?php echo ($my_role == 'Admin') ? 'or group ' : ''; ?>to start messaging
                    </h3>
                </div>
            </div>
        ...
        <div class="modal-content"
            ...
            <div style="padding: 20px 20px 10px 20px; border-bottom: 1px solid #e5e7eb;">
                ...
                <input type="text" id="newContactSearch" class="form-control" placeholder="Search by name or phone..."
                    style="margin-bottom: 15px; width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
            ...
            <div class="modal-content"
                ...
                <div style="padding: 15px 20px; border-top: 1px solid #e5e7eb;">
                    <button class="btn-add-blue"
                        style="width: 100%; padding: 12px; background: #1e3a8a; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: bold;"
    ...
    <input type="hidden" id="myUserRole" value="<?php echo $my_role; ?>">
    ...
    <script>
        window.myRole = "<?php echo $my_role; ?>";
    </script>
    ...
    <div id="imageLightbox"
        ...
        <a id="lightboxDownload" href="" download class="btn-add-blue"
            style="margin-top:20px; padding: 10px 20px; background: #1e3a8a; color: white; border-radius: 8px; text-decoration:none;"><i

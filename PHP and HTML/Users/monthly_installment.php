...
// ======================================================
// AUTO-MARK OVERDUE (runs on every page load)
// ======================================================
mysqli_query(
    $conn,
...
// ======================================================
// FETCH BOOKING + DP + INSTALLMENT SUMMARY
// ======================================================
...
$installments = [];
while ($r = mysqli_fetch_assoc($inst_q)) $installments[] = $r;
...
foreach ($installments as $i) {
    ...
    if ($i['payment_status'] === 'Overdue') $overdue_months++;
    if (
        $next_due_inst === null
...
// ======================================================
// CHECK BLACKLIST (overdue >= 21 days)
// ======================================================
...
// Determine pay mode: either GET ?pay=X or POST
if ($pay_inst_id > 0) {
    foreach ($installments as $i) {
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            mysqli_stmt_bind_param(
                $ins,
                "idsss",
                $booking_id, $inst_amt, $receipt_num, $pay_ref, $remarks
            );
...
.page-heading{ margin-bottom:24px; display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;}
.page-heading h1{
...
.back-btn:hover{ background:#f8fafc; border-color:#1e293b; }
...
@media(max-width:700px){ .vehicle-banner{ grid-template-columns:1fr; text-align:center; } }
.vehicle-banner img{
    width:160px; height:110px; object-fit:cover;
    border-radius:10px; border:1px solid #e2e8f0;
}
.vehicle-banner .name{ font-size:18px; color:#0f172a; font-weight:700; text-transform:uppercase; margin-bottom:4px; }
.vehicle-banner .meta{ font-size:12px; color:#64748b; }
...
/* Progress bar */
.progress-track{
    background:#e2e8f0; height:10px; border-radius:999px;
...
.progress-fill.warn{ background:linear-gradient(90deg,#f59e0b 0%,#dc2626 100%); }
...
.blacklist-warning i{ font-size:28px; color:#dc2626; }
.blacklist-warning .ti{ font-size:15px; font-weight:700; color:#991b1b; margin-bottom:4px; }
.blacklist-warning .ds{ font-size:13px; color:#7f1d1d; line-height:1.5; }
...
.inst-table tr.next-due td{ border-bottom-color:#fde68a; }
.inst-table tr.is-paid td{ color:#94a3b8; }
.inst-table tr:hover:not(.is-paid){ background:#f8fafc; }
...
.inst-amount{
    font-family:monospace; font-weight:700; color:#1e293b;
}
...
.btn-pay-row:hover{ background:#15803d; transform:translateY(-1px); }
.btn-pay-row.danger{ background:#dc2626; }
.btn-pay-row.danger:hover{ background:#991b1b; }

/* ============ Payment Form Section ============ */
.payment-modal-bg{
    position:fixed; inset:0; background:rgba(15,23,42,0.6);
    display:flex; align-items:flex-start; justify-content:center;
    z-index:1000; padding:30px 20px; overflow-y:auto;
}
.payment-modal{
...
.payment-modal .pm-header p{ font-size:12px; opacity:0.75; }
...
.payment-modal .pm-close:hover{ background:rgba(255,255,255,0.25); }
.payment-modal .pm-body{ padding:24px; }
...
.pay-amount-display .lbl{ font-size:11px; color:#92400e; font-weight:600; text-transform:uppercase; }
.pay-amount-display .num-disp{ font-size:11px; color:#78350f; }
.pay-amount-display .amt{ font-size:22px; font-weight:800; color:#dc2626; }
...
.card-visual .row-bottom .v{
    ...
    margin-top:2px; opacity:1; text-transform:none;
}
...
.btn-pay:hover:not(:disabled){ background:#0f172a; }
...
.error-box p{ margin:2px 0; font-size:12px; font-weight:500; }
.error-box p::before{
    ...
    <div class="mi-wrapper">
            ...
            <div class="vehicle-banner">
                ...
                <div>
                    ...
                    <div class="meta" style="margin-top:6px;">
                        <strong>Loan:</strong> <?= htmlspecialchars($booking['installment_years']) ?> Years @ <?= number_format($booking['interest_rate'], 2) ?>% p.a.
                    </div>
                </div>
        ...
        <div class="blacklist-warning">
            ...
            <div>
                ...
                <div class="ds">One or more of your installments is overdue by 21 days or more. Please contact our finance department immediately to resolve your account.</div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="progress-track" style="margin-top:8px;">
                    <div class="progress-fill <?= $overdue_months > 0 ? 'warn' : '' ?>" style="width:<?= $progress_pct ?>%"></div>
                </div>
            ...
            <div class="stat-card info">
                ...
                <div class="sub"><?= $total_months - $paid_months ?> month<?= ($total_months - $paid_months) > 1 ? 's' : '' ?> left</div>
            </div>
            ...
            <div class="head">
                ...
                <?php if ($next_due_inst && !$is_blacklisted): ?>
                    <div style="font-size:12px;color:#64748b;">
                        Next due: <strong style="color:#1e293b;"><?= date('d M Y', strtotime($next_due_inst['due_date'])) ?></strong>
                    </div>
                <?php endif; ?>
            </div>
            <table class="inst-table">
                ...
                <tbody>
                    ...
                    foreach ($installments as $i):
                        ...
                        if ($is_next) $next_pending_seen = true;

                        $row_class = '';
                        if ($is_paid) $row_class = 'is-paid';
                        elseif ($is_next) $row_class = 'next-due';
                    ?>
                    <tr class="<?= $row_class ?>">
                        ...
                        <td style="text-align:right;">
                            ...
                            <?php elseif ($is_next): ?>
                                <a href="?id=<?= $booking_id ?>&pay=<?= $i['installment_id'] ?>" class="btn-pay-row <?= $is_overdue ? 'danger' : '' ?>">
...
<div class="payment-modal-bg" id="paymentModal">
    <div class="payment-modal">
        <div class="pm-header">
            <h2><i class="fas fa-credit-card"></i> Pay Installment #<?= $pay_inst['installment_number'] ?> of <?= $total_months ?></h2>
